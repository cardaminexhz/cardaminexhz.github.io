 ...
 * @comment 点击提交按钮时，通过模拟focus, blur事件，来校验表单
 */
...
function checkAllValue() {
    var artifactFocusEvent = document.createEvent("MouseEvents"),
        artifactBlurEvent = document.createEvent("MouseEvents");

    // 模拟事件
    input = document.getElementsByTagName("input");
    for (var i = 0; i < input.length; i++) {
        if(input[i].type === "text" || input[i].type === "password") {
            artifactFocusEvent.initEvent("focus", true, true);
            artifactBlurEvent.initEvent("blur", true, true);
            input[i].dispatchEvent(artifactFocusEvent);
            input[i].dispatchEvent(artifactBlurEvent);
        }
    }
